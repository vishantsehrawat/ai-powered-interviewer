...
export const InterviewPage = () => {
    ...
    const { isLoading, questionSet } = useSelector(
    ...
    const handleSubmit = () => {
        setAllanswer([...allanswer, answer])
        const payload = generateScoreRequest();
        dispatch(generateInterviewScore(payload));
    }


    const generateScoreRequest = () => {

        let req = ""

        for (let i = 0; i < question.length; i++) {
            const ans = allanswer[i] ? allanswer[i] : " ";
            if (i == 0) {
                req = `question:${question[i]} \n answer:${ans} \n`
            } else {
                req += `question:${question[i]} \n answer:${ans} \n`
            }
        }

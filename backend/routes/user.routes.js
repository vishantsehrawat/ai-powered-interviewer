        ...
        try {
            if (myUser) {
                bcrypt.compare(user.password, myUser.password, function (err, result) { // eslint-disable-line no-unused-vars
                    console.log("🚀 ~ file: user.routes.js:53 ~ result:", result)
                    if (result) {
                        ...
                        res.status(200).send({ msg: "User logged in", token, refreshToken, usernameforchat: myUser.name, userId: myUser._id, wholeUser: myUser })
                    }
                    else {
                        res.status(400).send({ msg: "User password error", error: err })
                    }
                });
            }
            else {
